...
const PiecePhaseIncrement = enum(u8) { Pawn = 0, Knight = 1, Bishop = 1, Rook = 2, Queen = 4, King = 0 };
...
const mg_pst: [6][64]i32 = blk: {
    ...
    for (0..6) |pt| {
        for (0..64) |sq| {
            tables[pt][sq] = mg_value[pt] + raw[pt][sq];
        }
    }
    break :blk tables;
};

const eg_pst: [6][64]i32 = blk: {
    var tables: [6][64]i32 = undefined;
    const raw = [6][64]i32{ eg_pawn_table, eg_knight_table, eg_bishop_table, eg_rook_table, eg_queen_table, eg_king_table };
    for (0..6) |pt| {
        for (0..64) |sq| {
            tables[pt][sq] = eg_value[pt] + raw[pt][sq];
        }
    }
    break :blk tables;
};
